...
void ChartiumLegendMoveResizeHandler::handleMouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    else if (m_action == Action::Resizing)
    {
        ...
        geom = QRectF(
            geom.topLeft(), geom.size().expandedTo(m_legend->layout()->minimumSize()).boundedTo((boundRect & geom).size())
        );
...
{
    const QList<QGraphicsItem*> items = m_legend->items()->childItems();
...
{
    QRectF contentRect = m_legend->layout()->contentsRect();
