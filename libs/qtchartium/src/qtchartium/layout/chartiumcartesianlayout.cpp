...
constexpr qreal MAX_AXIS_PORTION = 0.4;



ChartiumCartesianLayout::ChartiumCartesianLayout(IChartiumPresenter* presenter, QGraphicsLayoutItem* parent) :
    IChartiumCartesianLayout(presenter, parent)
{
}

ChartiumCartesianLayout::~ChartiumCartesianLayout()
{
}

QRectF ChartiumCartesianLayout::calculateAxisGeometry(
    const QRectF& geometry, const QList<IChartiumAxisElement*>& axes, bool update
) const
{
    QSizeF left(0, 0);
    QSizeF minLeft(0, 0);
    ...
    for (IChartiumAxisElement* axis : axes)
    {
        if (!axis->isVisible())
        {
            continue;
    ...
    {
        vratio = (MAX_AXIS_PORTION * geometry.width()) / totalVerticalAxes;
    }
    ...
    {
        hratio = (MAX_AXIS_PORTION * geometry.height()) / totalHorizontalAxes;
    }
    ...
    QRectF chartRect = geometry.adjusted(
        qMax(left.width(), minWidth / 2),
        qMax(top.height(), minHeight / 2),
        -qMax(right.width(), minWidth / 2),
        -qMax(bottom.height(), minHeight / 2)
    );
    ...
    if (mPresenter->isFixedGeometry())
    {
        chartRect = mPresenter->geometry();
    }
    ...
    for (IChartiumAxisElement* axis : axes)
    {
        ...
        switch (axis->axis()->alignment())
        {
            ...
            case Qt::AlignTop:
            {
                ...
                axis->setGeometry(
                    QRect(geometry.left(), chartRect.top() - topOffset - height, geometry.width(), height), chartRect
                );
                topOffset += height;
                break;
            }
...
QRectF ChartiumCartesianLayout::calculateAxisMinimum(const QRectF& minimum, const QList<IChartiumAxisElement*>& axes) const
{
    QSizeF left;
    QSizeF right;
    QSizeF bottom;
